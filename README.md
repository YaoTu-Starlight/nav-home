...
本项目包含一个自定义的后处理脚本 `scripts/make-dist.js`，它会将 Next.js 的 Standalone 产物打包成一个独立的 `dist` 文件夹，非常适合部署到 VPS 环境。
...
**构建流程说明**：
脚本会自动清理旧目录，复制 standalone 文件、静态资源及数据文件，并**自动锁定端口为 3300**。
...
1.  **上传**: 将 `dist` 文件夹复制到服务器任意位置。
2.  **启动**: 进入目录并运行 server (无需再次安装依赖)。
    ```bash
    cd dist
    node server.js
    ```
3.  **后台运行 (推荐)**: 使用 PM2 管理进程。
    ```bash
